...
describe("OpenSesame", function () {
    ...
    describe("Deployment", function () {
        ...
        it("Should be able to correctly verify proofs", async function () {
        ...
        it("Should be able to verify a given player knows correct answer", async function () {
            let { contract, owner, alice, bob, charlie } = await loadFixture(deployFixture);
            await contract.connect(alice).registerPlayer();
            await contract.connect(bob).registerPlayer();
            await contract.connect(alice).guruProof(alice.address, validZKProofs.alice.proof, validZKProofs.alice.pubSig.slice(1));
        });
        ...
        it("Should be able to receive treasury if knows answer", async function () {
            ...
            await contract.connect(alice).claimTreasury(alice.address);
        ...
        it("Should be REJECT answer if answers was for another player", async function () {
            let { contract, owner, alice, bob, charlie } = await loadFixture(deployFixture);
            ...
            await expect(contract.connect(bob).guruProof(alice.address, validZKProofs.alice.proof, validZKProofs.alice.pubSig.slice(1)))
                .to.be.rejectedWith("Proof verification failed");
                await expect(contract.connect(alice).claimTreasury(alice.address)).to.be.rejectedWith("Only gurus can claim the treasury");
        });
    });
